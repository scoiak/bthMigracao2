...
from (
	 select
	 	...
	 	(case when e.edtdataentrprop is null then null else concat(coalesce(e.edtdataentrprop, m.mindata, p.pcsdataproc), ' ', coalesce(e.edthoraentrprop, '00:00:00')) end)::varchar as dh_andamento,
	 	null as observacao,
	 	m.cmlcodigo,
	 	...
	 	(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'processo', m.clicodigo, m.minano, m.pcsnro)))  as id_processo,
	 	(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'processo-forma-contratacao', m.clicodigo, m.minano, m.pcsnro))) as id_forma_contratacao,
