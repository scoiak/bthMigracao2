...
from (
	(select
		...
		(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'solicitacao-recebimento', i.clicodigo, i.copano, i.copnro))) as id_recebimento,
		(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'solicitacao-recebimento-item', i.clicodigo, i.copano, i.copnro, '@', i.itcitem))) as id_gerado
...
union all
	(select distinct
		...
		(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'solicitacao-recebimento', i.clicodigo, i.copano, i.copnro))) as id_recebimento,
		(select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'solicitacao-recebimento-item', i.clicodigo, i.copano, i.copnro, '@', ic.cmiid))) as id_gerado
	from wco.tbitemnota i
	inner join wco.tbcompra c using (clicodigo, copano, copnro)
