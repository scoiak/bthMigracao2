...
from (
	select distinct
       ...
       (case
       		...
       		when mintipojulgamento = 4 then 'Maior Lance ou Oferta'
       ...
       'NENHUMA' as formaEspecial,
        (select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat(305, 'forma-julgamento', clicodigo, mintipojulgamento))) as id_gerado
	from wco.tbminuta
