...
sistema = 305
tipo_registro = 'ato'
url = 'https://compras.betha.cloud/compras-services/api/atos'
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        ...
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, item['nro_ato'], item['ano_ato'], item['tipo_ato'])
        dict_dados = {
        ...
        if item['data_publicacao'] is not None:
            dict_dados.update({'dataPublicacao': item['data_publicacao'].strftime("%Y-%m-%d")})

        if item['data_vigorar'] is not None:
            dict_dados.update({'dataVigorar': item['data_vigorar'].strftime("%Y-%m-%d")})
