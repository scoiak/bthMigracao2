...
sistema = 305
tipo_registro = 'processo-participante'
url = 'https://compras.betha.cloud/compras-services/api/exercicios/{exercicio}/processos-administrativo/{processoAdministrativoId}/participante-licitacao'
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        ...
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, item['clicodigo'], item['ano_processo'],
                                              item['nro_processo'], item['cpf_fornecedor'])
        ...
        dict_dados = {
            ...
            'dhCredenciamento': item['dh_credenciamento'],
            'declaracaoMPE': item['declaracao_mpe'],
            'situacaoDocumentacao': {
                'valor': item['situacao_documentacao']
            },
            'renunciouRecurso': item['renunciou_recurso'],
            'sedeMpe': {
                'valor': item['sede_mpe']
            },
        ...
        if item['nome_representante'] is not None:
            dict_dados.update({'nomeRepresentante': item['nome_representante']})
        ...
        lista_controle_migracao.append({
            ...
            'descricao_tipo_registro': 'Cadastro de Participantes do Processo',
            'id_gerado': None,
