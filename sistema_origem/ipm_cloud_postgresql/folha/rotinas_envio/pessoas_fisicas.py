...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        hash_chaves = model.gerar_hash_chaves('1', 'pessoas_fisicas', item['codigo'])
        dict_dados = {
            'idIntegracao': hash_chaves,
            'conteudo': {
                'nome': item['nome'],
                'cpf': item['cpf'],
                'dataNascimento': item['data_nascimento'].strftime("%Y-%m-%d"),
                'sexo': item['sexo']
            }
        }
        ...
        lista_controle_migracao.append({
            'sistema': 1,
            'tipo_registro': 'pessoas_fisicas',
            'hash_chave_dsk': hash_chaves,
            'descricao_tipo_registro': 'Cadastro de Pessoas Físicas',
            'id_gerado': None,
