...
sistema = 300
tipo_registro = 'base'
url = 'https://pessoal.cloud.betha.com.br/service-layer/v1/api/base'
limite_lote = 1000
...
def busca_dados(params_exec):
    ...
    for item in registros:
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, '56', item['sigla'])
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        ...
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, item['entidade'], item['sigla'])
        dict_dados = {
            'idIntegracao': hash_chaves,
            'conteudo': {
                'descricao': item['descricao'],
                'sigla': item['sigla'],
                'classificacaoBaseCalculo': item['classificacaobasecalculo']
            }
        }
        ...
        lista_controle_migracao.append({
            ...
            'tipo_registro': tipo_registro,
            'hash_chave_dsk': hash_chaves,
            'descricao_tipo_registro': 'Cadastro do Tipo de Base',
            'id_gerado': None,
            'json': json.dumps(dict_dados),
            'i_chave_dsk1': item['entidade'],
            'i_chave_dsk3': item['sigla']
        })
