...
select 
	...
	(select string_agg(suc.fgodatainicio || '%|%' || suc.fgodatafinal || '%|%' || '' || '%|%' || '' || '%|%' || suc.fgodatainicio || '%|%' || suc.fgodatafinal || '%|%' || suc.fgodatainicio || '%|%' || '' || '%|%' || suc.fgodiasgozo || '%|%' || '' || '%|%' || '','%||%') from (select * from wfp.tbferiasgozada as fg where fg.ferdatainicio = pf.ferdatainicio and fg.odomesano = pf.odomesano and fg.fcncodigo = pf.fcncodigo) as suc) as movimentacoes
from 
