...
from (
	select
		...
		/* 30 */ regexp_replace((array(select distinct unnest(string_to_array((
			select
				string_agg(concat(nv2.nivcodigo, ': ', coalesce((select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300', 'nivel-salarial', nv2.nivcodigo)))::text, '?')), ', ')
		...
		), ', ')))::text), '[\,]', '|', 'gi')::text as niveis_vigentes
	from (
		select
			nv.carcodigo,
			nv.cnidatarelaciona,
...
from (
	select
		...
		coalesce((select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300', 'tipo-cargo', c.cartipocargo))), 0) as id_tipo_cargo,
