...
select distinct
       id,
       ano,
       left(((case when nivel = '1' then '' else ano::varchar end) || organograma || '000000000000000'),12)  as numero,
       nivel,
       descricao,
       sigla,
       (select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300', 'entidade', 11968))) as entidade,
       (select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300','configuracao-organograma', (select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300', 'configuracao-organograma', 11968))), 1))) as configuracao
from (
	select distinct
		ano::text as id,
		ano::smallint as ano,
...
where nivel::int >= 1
  and LENGTH(numero) <= 15
  and ano <= 2020
  and (select id_gerado from public.controle_migracao_registro where hash_chave_dsk = md5(concat('300','organograma',entidade,configuracao,numero))) is null
  order by  
  nivel asc,
  ano asc
